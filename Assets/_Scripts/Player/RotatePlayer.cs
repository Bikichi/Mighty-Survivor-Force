...
public class RotatePlayer : MonoBehaviour
{
    ...
    public void RotateInMoveDirection(Transform transform)
    {
        ...
        moveDirection.y = 0;
        // Chỉ xoay trục y mà không muốn xoay các trục xz bằng cách đặt y = 0
    ...
    {
        Vector3 directionToEnemy = CheckDistance.Instance.FindClosestEnemy().position - transform.position;
    ...
    public void RotatePlayerInDirection()
    {
        ...
        // Tính khoảng cách và check tầm đánh từ PlayerStats gắn trên prefab
        if (targetEnemy != null && playerStats != null)
        {
            float distance = CheckDistance.Instance.CalculateDistanceToPlayer(transform, targetEnemy);
            canSeeEnemy = distance <= playerStats.baseAttackRange;
        }
        ...
        if (targetEnemy != null && canSeeEnemy)
        {
            if (isReadyToRotate)
            {
                ...
                // targetEnemy không đổi thì hàm này thực chất vẫn chạy chỉ cần thoả mãn điều kiện if
