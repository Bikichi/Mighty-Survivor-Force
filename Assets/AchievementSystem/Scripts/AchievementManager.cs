...
public class AchievementManager : MonoBehaviour
{
    ...
    public void Unlock(string Key)
    {
        ...
        CoinManager.Instance.totalCoinValue += 100;
    ...
    private void DisplayUnlock(int Index)
    {
        if (DisplayAchievements && (!AchievementList[Index].Spoiler || States[Index].Achieved))
        {
            ...
            {
                int notificationFreq = (int)AchievementList[Index].NotificationFrequency;

                // Bỏ qua nếu NotificationFrequency bằng 0
                if (notificationFreq <= 0)
                {
                    notificationFreq = 1; // hoặc bạn muốn skip luôn
                }

                int Steps = (int)AchievementList[Index].ProgressGoal / notificationFreq;

                for (int i = Steps; i > States[Index].LastProgressUpdate; i--)
                {
                    if (States[Index].Progress >= notificationFreq * i)
                    {
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.R))
        {
            ...
            Debug.Log("Đã reset tất cả thành tựu!");
        }
    }
